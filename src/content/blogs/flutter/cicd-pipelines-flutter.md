...
Generate one at: **App Store Connect → Users and Access → Integrations → App Store Connect API**
...
Store it as `fastlane/api_key.json` and add it to `.gitignore`. We'll cover the exact format of that file in Step 5.
...
└── play-store-credentials.json       ← service account key (GITIGNORED — never commit)
```
